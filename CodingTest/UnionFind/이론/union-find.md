...
`union`(합집합) 연산은 두 원소가 포함된 집합을 하나의 집합으로 합치는 연산이고, `find`(찾기) 연산은 특정 원소가 속한 집합이 어떤 집합인지 알려주는 연산이다.
...
실제로 구현할 때는 `a`와 `b` 중에서 더 번호가 작은 원소가 부모 노드가 되도록 구현하는 경우가 많다. 아래 예시도 이러한 방식을 따르도록 하겠다.
