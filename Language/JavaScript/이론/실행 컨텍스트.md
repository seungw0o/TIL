...
클로저는 실행 컨텍스트와 렉시컬 환경의 작동 원리로 인해 발생하는 현상입니다.
...
1. `outer` 함수의 렉시컬 환경에는 변수 `x`와 함수 `inner`가 저장됩니다.
2. `inner` 함수는 자신의 Outer Lexical Environment Reference를 통해 `outer` 함수의 렉시컬 환경을 참조합니다.
3. `outer` 함수가 종료된 후에도 `inner` 함수가 외부 렉시컬 환경을 참조하고 있어, `x`에 접근할 수 있습니다.
4. 이처럼 외부 함수의 변수를 내부 함수가 참조할 수 있는 것이 클로저입니다.
