...
```jsx
// 텍스트 필드의 문자를 자동 저장하여 브라우저가 의도치 않게 재시작되었을 경우,
// 저장된 문장으로 복구해 작성한 내용이 사라지지 않게 한다.
...
- 서버는 클라이언트의 로그인 요청에 대한 응답을 작성할 때 인증 정보는 서버에 저장하고, 클라이언트 식별자인 `JSESSIONID`를 쿠키에 담는다. 이후 클라이언트는 요청을 보낼 때마다 `JSESSIONID` 쿠키를 함께 보낸다. 그리하면 서버는 `JSESSIONID`의 유효성을 판별해 클라이언트를 식별한다.
...
- Access Token은 보호된 정보에 접근할 수 있는 권한 부여에 사용된다.
  권한을 부여받는 데에는 Access Token만 있으면 된다. 하지만 해킹의 위험성이 있기 때문에 Access Token은 일정 기간이 지나면 만료되어 버린다.

#### Refresh Token

- Access Token이 만료될 때마다 Refresh Token이 Access Token을 재발급해 주는 역할을 한다. Access Token이 열쇠라면, Refresh Token은 열쇠 재발급 같은 개념이다.
...
- `OAuth`의 경우 Facebook, Google 등 소셜 계정을 이용해 다른 웹 서비스에서도 로그인할 수 있다.
...
- 토큰을 탈취당하면 대처하기 어렵다. 토큰은 한 번 발급되면 유효기간이 만료될 때까지 계속 사용할 수 있다.
...
- 토큰의 만료 기한을 짧게 설정해서 토큰이 탈취되더라도 빠르게 만료시키는 방법이다. 하지만 토큰이 만료되면 사용자가 다시 로그인해야 하기 때문에 사용자 입장에서는 번거롭다.
